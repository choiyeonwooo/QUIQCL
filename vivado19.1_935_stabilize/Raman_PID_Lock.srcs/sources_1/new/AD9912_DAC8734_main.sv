...
*/
    ...
    always @ (posedge CLK100MHZ) begin 
           if(CLK_USER_cnt=='d25000) begin // 2kHz user clock (used for ADC data sampling)
               CLK_USER_cnt<='d0;
               CLK_USER<= ~CLK_USER;
           end
           else
               CLK_USER_cnt <= CLK_USER_cnt+'d1; // sclk_ADC 에  clock 을 넣어주지 않고 이렇게 카운팅만 해주면서 1클락을 소비하니 그 클락은 0 인 것. 이런식으로 9개 클럭 꺼서 100MHz를 1/10 해줌
       end
