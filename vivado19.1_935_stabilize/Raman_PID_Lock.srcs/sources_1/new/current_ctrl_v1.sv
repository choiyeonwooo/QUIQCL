...
module main(
    ...
    reg [DDS_WIDTH + 8:1] user_DDS_buffer; // user buffer(receives data related to DDS, then transfer to DDS_buffer)
            ...
            case (con_state)
                ...
                CON_UPDATE: begin 
                    if ((CMD_Length == $bits(CMD_UPDATE)/8) && (CMD_Buffer[$bits(CMD_UPDATE):1] == CMD_UPDATE)) begin //when command come intermidiate
                        ...
                        // COMMAND "WRITE DDS REG" - always comes with DDS_on
                        if ((CMD_Length == $bits(CMD_DDS_WRITE)/8) && (CMD_Buffer[$bits(CMD_DDS_WRITE):1] == CMD_DDS_WRITE)) begin
                            if (BTF_Length != (DDS_MAX_LENGTH+1)) begin
                                ...
                                //user_DDS_buffer -> DDS_buffer @OP_DDS state if DDS_ready_flag==0 and some specific condition
                            end
                        end    
...
///////////////////////////////////////////
    ...
    always @ (posedge CLK100MHZ)        
        case (op_state)
            ...
            //             adc_out_buf[17:0] <= ADC_output[17:0]; //we store the ADC data to buf
            ...
            OP_COMP_WAIT: begin
                ...
                if (PID_mode == 1'b0) begin
                    ...
                    if (COMP_on == 1'b1) begin // while locking(COMP_on), no DDS setting update allowed.
                        op_state <= OP_AOM_Feedback;
                    end
                    else if (DDS_on == 1) begin // when there's something to write on DDS
                        op_state <= OP_DDS;
                    end
                    else begin
                        op_state <= OP_WAIT;
            ...
            OP_DDS: begin
                ...
                if(PID_mode == 1'b0) begin
                    ...
                    if(DDS_ready_flag == 1'b0) begin
                        DDS_ready_flag <= 1'b1;
                        DDS_buffer[DDS_WIDTH + 8:1] <= user_DDS_buffer[DDS_WIDTH + 8:1];
                    end
                    else if(DDS_busy_2 == 0) begin
                        if((DDS_buffer[DDS_WIDTH + 4 : DDS_WIDTH + 1] == 4'b0100) && (DDS_buffer[DDS_WIDTH + 5] == 1'b1) && (DDS_buffer[DDS_WIDTH:DDS_WIDTH-15]==16'h240C)) 
                            current_FSC_AOM <= DDS_buffer[FTW_WIDTH+FSC_WIDTH-16:FTW_WIDTH-15]; // save FSC for AOM
                        dds_data_ready_2 <= DDS2_update;
                    end
                    else begin
                        DDS_ready_flag <= 1'b0;
