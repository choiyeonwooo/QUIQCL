...
class Raman_PID_Controller_GUI(QtWidgets.QDialog, Ui_QDialog):
    ...
    def initUi(self):
        ...
        # PID control Combobox setting
        self.ctrl_variable_list = self.config['control_variable']['variable_list'].split(', ')

        cvar_number = len(self.ctrl_variable_list)
        for i in range(0, cvar_number): #Combobox initialization
        ...
        self.setpoint_box.returnPressed.connect(lambda:self.PID.set_setpoint(float(self.setpoint_box.text())))
    ...
    def ADC_read(self):
        freq_difference = self.PID.adc_load_large_data_Freq()
    ...
    def freq_apply(self): # frequency apply (tracking + AOM)
        ...
        try:
            ...
            self.PID.set_frequency(tracking_freq, 1, 0) # ch1 set freq
            self.PID.set_frequency(aom_freq, 0, 1) # ch2 set freq
            return aom_freq
        except:
            return
    
    # def current_apply(self):
    #     try:
    #         tracking_current = float(self.track)
    ...
    def real_time_data_read(self):      
         real_time_data = self.PID.adc_load_data()
         ...
         freq_aom_MHz = real_time_data[2] - self.offset # Preserve original value by substracting offset
                                                        # current if pid mode = 1
         if (self.PID.pid_mode == 0):
